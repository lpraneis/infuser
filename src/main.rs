...
infuser:
long_running_command | infuser run \"Important Line\" /dev/pts/X
...
async fn main() -> anyhow::Result<()> {
    ...
    match args.mode {
        ...
        OperationMode::Update { new_filter } => run_utility(&args.sock_name, new_filter).await,
    }
}

async fn run_utility(sock: &str, filter: String) -> anyhow::Result<()> {
...
async fn run_input(sock: &str, tty: &str, inital_filter: String) -> anyhow::Result<()> {
    let mut re = Regex::new(&inital_filter).unwrap();
